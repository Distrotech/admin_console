...
//  FaxConf faxconf=new FaxConf();
  DefaultMutableTreeNode topbranch,intNode,iwconfig,lwireless,wanNode,genroute,grenode,espnode,espaccnode,modemnode,rulenode,modemrules,lnetwork,sslcert;
//protoconf,faxnode
...
//          (service.getText().length() > 0)){
  ...
  class NetworkGlobal extends Container implements ActionListener {
    JComboBox lint,lent;
//applenetphase;
    JTextField prwins,secwins,nexthop,lease,maxlease,ntpserver,pdns,sdns,ldaplogin,ldapserver,vpnrange;
...
*/
  ...
  class RouteConfig extends Container implements ActionListener {
    ...
    public RouteConfig(DefaultMutableTreeNode actnode,boolean edit) {
      ...
      if (isEdit) {
        ...
        intgw.setText(EditRoute.IPGateway);
  ...
  class WanConfig extends Container implements ActionListener {
    ManageNode sortpanel;
    JTextField intdescrip,intip,intgw,intstart,intend,intlgw,intrgw; 
    JLabel textlabel;
    boolean isEdit;
    WanDef EditWan;
    DefaultMutableTreeNode node = (DefaultMutableTreeNode)intwindow.getLastSelectedPathComponent();
    ...
    public WanConfig(boolean edit) {
      ...
      if (isEdit) {
        ...
        intip.setText(EditWan.IPAddress+"/"+EditWan.IPSubnet);
        intgw.setText(EditWan.IPGateway);
    ...
    public void actionPerformed(ActionEvent event) {
      ...
      if ((ipdata[0].length() > 0 ) & (ipdata[1].length() > 0) & 
        ...
        if (! isEdit) {
          DefaultMutableTreeNode childnode=addIpRoute(intdescrip.getText(),ipdata[0],ipdata[1],
          ...
          EditWan.IPAddress=ipdata[0];
          EditWan.IPSubnet=ipdata[1];
...
/*
  class FaxConfig extends Container implements ActionListener {
    JComboBox port,speed,flow,trig;
    JCheckBox conwait,volume,stream,ctimeout;
    JTextField tagname,tagnum,ccode,acode,ldist,intpre,retry,number,ringdelay,flowcmd,dtrcmd,dcdcmd,rtimeout,cidno,cidna;
    ...
    public FaxConfig() {
      ...
      add(portlabel);
      layout.gridwidth=GridBagConstraints.REMAINDER;
      String portlist[]={"Disabled","Com1","Com2","Com3","Com4","Isdn1","Isdn2","Isdn3","Isdn4"};
      port=new JComboBox(portlist);
      port.setSelectedItem(faxconf.ComPort);
      gridbag.setConstraints(port,layout);
      add(port);
      ...
      String speedlist[]={"115200","57600","38400","19200","9600"};
      speed=new JComboBox(speedlist);
      speed.setSelectedItem(faxconf.Speed);
      gridbag.setConstraints(speed,layout);
      add(speed);

      layout.gridwidth=1;
      JLabel triglabel=new JLabel("Receive Data Triger");
      gridbag.setConstraints(triglabel,layout);
      add(triglabel);
      layout.gridwidth=GridBagConstraints.REMAINDER;
      String triglist[]={"DC1","DC2"};
      trig=new JComboBox(triglist);
      trig.setSelectedIndex(faxconf.Trigger);
      gridbag.setConstraints(trig,layout);
      add(trig);
      ...
      add(flowlabel);
      layout.gridwidth=GridBagConstraints.REMAINDER;
      String flowlist[]={"Hardware (RTS/CTS)","Software (XON/XOFF)"};
      flow=new JComboBox(flowlist);
      flow.setSelectedIndex(faxconf.FlowControl);
      gridbag.setConstraints(flow,layout);
      add(flow);
      ...
      flowcmd=new JTextField(faxconf.FlowCmd,10);
      addLabel(new JLabel("Modem Flow Control Command"),flowcmd,gridbag,layout);

      dtrcmd=new JTextField(faxconf.DtrCmd,10);
      addLabel(new JLabel("Modem DTR Command"),dtrcmd,gridbag,layout);

      dcdcmd=new JTextField(faxconf.DcdCmd,10);
      addLabel(new JLabel("Modem DCD Command"),dcdcmd,gridbag,layout);
 
      layout.fill=GridBagConstraints.NONE;
      layout.gridwidth=1;
      conwait=new JCheckBox("Wait For CONNECT (Ignore DATA/FAX)",faxconf.ConectWait);
      gridbag.setConstraints(conwait,layout);
      add(conwait);

      layout.gridwidth=GridBagConstraints.REMAINDER;
      volume=new JCheckBox("Enable Modem Volume",faxconf.Volume);
      gridbag.setConstraints(volume,layout);
      add(volume);

      layout.fill=GridBagConstraints.NONE;
      layout.gridwidth=1;
      stream=new JCheckBox("Disable Stream Mode",faxconf.Stream);
      gridbag.setConstraints(stream,layout);
      add(stream);

      layout.gridwidth=GridBagConstraints.REMAINDER;
      ctimeout=new JCheckBox("Disable Phase C Timeout Config.",faxconf.CTimeout);
      gridbag.setConstraints(ctimeout,layout);
      add(ctimeout);
    ...
    public void actionPerformed(ActionEvent event) {
      faxconf.ComPort=port.getSelectedItem().toString();
      faxconf.Speed=speed.getSelectedItem().toString();
      faxconf.FlowControl=flow.getSelectedIndex();
      faxconf.Trigger=trig.getSelectedIndex();
      faxconf.Volume=volume.isSelected();
      faxconf.ConectWait=conwait.isSelected();
      faxconf.Stream=stream.isSelected();
      faxconf.CTimeout=ctimeout.isSelected();
      ...
      faxconf.FlowCmd=flowcmd.getText();
      faxconf.DtrCmd=dtrcmd.getText();
      faxconf.DcdCmd=dcdcmd.getText();
...
*/
  public class AddFwUser extends Container implements ActionListener{
    JTextField username,address,intmac,snmask,intip,ingress,egress,intnm,intstart,intend,inteth,intingress,integress;
    JComboBox area;
    ManageNode sortpanel;
    IntDef intdata;
    DefaultMutableTreeNode node = (DefaultMutableTreeNode)intwindow.getLastSelectedPathComponent();
    ...
    public AddFwUser(){
      ...
      if (node != modemrules) {
        ...
        intip=new JTextField(intdata.IPAddress,10);
        addLabel(new JLabel("Interface Ip Address"),intip,gridbag,layout);

        intnm=new JTextField(intdata.IPSubnet,10);
    ...
    public void actionPerformed(ActionEvent event) {
      ...
      if (event.getActionCommand() == "Save Interface") {
        intdata.IPAddress=intip.getText();
        intdata.IPSubnet=intnm.getText();
    ...
    public void actionPerformed(ActionEvent event) {
      ...
      if (event.getActionCommand() == "Add New Rule") {
        ...
        if ((destip.getText().length() > 0) & (discrip.getText().length() > 0)) {
          ...
          if ((! iface.toString().equals("Modem")) & (! iface.toString().equals("GRE Tunnel"))) {
      ...
      if ((servalid) && (! systype.equals("full"))) {
        ...
        if ((intset.IPStart.length() > 0) & (intset.IPEnd.length() >0 )) {
          ...
          Output=AddConfL("IP Interface "+intset.Description.replaceAll(" ","_")+" "+intset.IPAddress+" "+intset.IPSubnet+" "+intset.IntName+" - - "+
                          intset.Ingress+" "+intset.Egress+" "+intset.MAC);
        }
      } else {
        for (Enumeration e = intNode.children() ; e.hasMoreElements() ;) {
          intdata=(DefaultMutableTreeNode)e.nextElement();
          ...
          if (intset.IntName.indexOf(":") < 0) {
            if ((intset.IPStart.length() > 0) & (intset.IPEnd.length() >0 )) {
              Output=AddConfL("IP Interface "+intset.Description.replaceAll(" ","_")+" "+intset.IPAddress+" "+intset.IPSubnet+" "+
                              intset.IntName+" "+intset.IPStart+" "+intset.IPEnd+" "+intset.Ingress+" "+intset.Egress+" "+intset.MAC);
            } else {
              Output=AddConfL("IP Interface "+intset.Description.replaceAll(" ","_")+" "+intset.IPAddress+" "+intset.IPSubnet+" "+intset.IntName+" - - "+
                              intset.Ingress+" "+intset.Egress+" "+intset.MAC);
            }
            ...
            if ((intset.IPStart.length() > 0) & (intset.IPEnd.length() >0 )) {
              Alias=Alias+"IP Interface "+intset.Description.replaceAll(" ","_")+" "+intset.IPAddress+" "+intset.IPSubnet+" "+
                    intset.IntName+" "+intset.IPStart+" "+intset.IPEnd+" "+intset.Ingress+" "+intset.Egress+" "+intset.MAC+newline;
            } else {
              Alias=Alias+"IP Interface "+intset.Description.replaceAll(" ","_")+" "+intset.IPAddress+" "+intset.IPSubnet+" "+
                    intset.IntName+" - - "+intset.Ingress+" "+intset.Egress+" "+intset.MAC+newline;
            }
          }
        }
      }
...
*/    
    ...
    String vpnrange;    
    DefaultMutableTreeNode intint;
    
    DefaultMutableTreeNode extint;
...
class voipdefreg {
  ...
  public String getconfig() {
    ...
    String newline = System.getProperty("line.separator");
    if (vboxip.length() > 0) {
      Output="IP VOIP VBOX "+vbox+" "+vboxpass+" "+vboxip+" "+vboxp+" "+vboxpre+" "+h323gkid+" "+vboxreg.toString();  
    }
